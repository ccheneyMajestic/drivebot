...
//    #define MJL_DEBUG_MOTORS            /* Test the motors */
    #define MJL_DEBUG_BOOTLOADABLE     /* Test the bootloader */
#endif
/* -------------- END DEBUG CASE --------------  */
...
int main(void) {
    ...
    for(;;){
        ...
        uart_println(&usb, "Press 'b' to enter bootloader mode");
        ...
        for(;;) {
            ...
            if(!readError) {          
                ...
                if('\r' == readVal) {
                    uart_print(&usb, "\r\nResetting...");
                    CyDelay(1000);
                    /* Schedule the application (this) to run after reset */
                ...
                /* Enter Bootloader mode */
                    ...
                    uart_print(&usb, "\r\nEntering Bootloader...");
                    CyDelay(200);
                    /* Schedule the Bootloader to run after reset */
...
//                    drivebot_motor_set_velocity(&state.motor_left, -400);
    ...
    #elif defined MJL_DEBUG_BOOTLOADABLE    
      /* Test the bootloader */
        uart_printHeader(&usb, "MJL_DEBUG_BOOTLOADABLE", __DATE__, __TIME__);
        CyDelay(10);
        uart_println(&usb, "Press 'enter' to reset the device");
        uart_println(&usb, "Press 'b' to enter bootload mode");
        uart_println(&usb, "");
        ...
        for(;;) {
            ...
            if(!readError) {
                ...
                if('\r' == readVal) {
                    ...
                    uart_print(&usb, "\r\Entering Bootloader...");
                    /* Schedule the Bootloader to run after reset */
                    Bootloadable_SET_RUN_TYPE(Bootloadable_SCHEDULE_BTLDR);
                    CySoftwareReset();
                }
